...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is None:
        ...
        for i in tickers_lst:
            if i not in tic_exchange_dic:
                raise Exception("{} dose not correspond to a key tic_exchange_dic".format(i))
...
def verify_cols(col_lst=None):
    ...
    if col_lst is None:
        ...
        for i in col_lst:
            if i not in COLUMNS:
                raise Exception("{} dose not found in 'COLUMNS".format(i))
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        ...
        for line in read_dat(tic):
            record = {}
            for col in col_lst:
                record[col] = line_to_dict(line).get(col)
            data.append(record)

        tic_dict['data'] = data
        rs_dict[tic] = tic_dict
